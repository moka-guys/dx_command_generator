...
def main():
    ...
    # Detect project information from DNAnexus file
    if args.dxfile:
        project_id, project_name = detect_project_info(args.dxfile)
...
""")
    ...
    if args.dxfile:
        sample_file = extract_samples_from_dx_file(args.dxfile)
    ...
    # Process a single sample if provided
    if args.sample:
        if process_sample(args.sample, args.output, args.failures, project_id, project_name):
            print(f"Command for sample '{args.sample}' has been written to '{args.output}'")
            processed += 1
        else:
            print(f"Failed to process sample '{args.sample}'")
            failed += 1
    
    # Process samples from file if provided
    if sample_file:
        if not os.path.isfile(sample_file):
            print(f"Error: Sample file '{sample_file}' not found!")
            sys.exit(1)
        
        print(f"Processing samples from file: {sample_file}")
        
        with open(sample_file, 'r') as f:
            samples = [line.strip() for line in f if line.strip() and not line.startswith('#')]
        
        total_samples = len(samples)
        print(f"Found {total_samples} samples in the file.")
        
        for i, sample in enumerate(samples):
            print(f"[{i+1}/{total_samples}] Processing sample: {sample}")
            if process_sample(sample, args.output, args.failures, project_id, project_name):
                processed += 1
            else:
                failed += 1
            print("--------------------------------------------")
